...
difference(){
    ...
    // unORIGINAL PIRANHA text
    translate([-74  ,   51  ,   0.6]) rotate([180,0,0]) linear_extrude(height=2) 
        text("unORIGINAL", font="helvetica:style=Bold", size=7);
    translate([-14  ,   51  ,   0.6]) rotate([180,0,0]) linear_extrude(height = 2) 
        text("PIRANHA", font="helvetica:style=Bold", size=11);
    translate([ -73 ,   40.5 , -0.4]) cube([57,1.6,1]);  
    translate([ -73 ,   41.3 , -0.4]) cylinder(h=1, r=0.8, $fn=30);  
    translate([ -16 ,   41.3 , -0.4]) cylinder(h=1, r=0.8, $fn=30);  
    ...
    translate([-73,15,4]) rotate([90,0,90]) linear_extrude(height = 2) 
        text("R1",font = "helvetica:style=Bold", size=7);
}
