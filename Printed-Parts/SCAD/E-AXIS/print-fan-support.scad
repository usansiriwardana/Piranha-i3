...
translate([0, 0, 8])rotate([0, 90, 0])difference(){
    ...
    union(){
        ...
        translate([4,-10,0]) rotate([0,0,0]) cube([4,15,30]);
        translate([0, -5,0]) rotate([0,0,0]) cube([8,10,5]);
        translate([0, -1,22]) rotate([48,0,0]){
        	cylinder(h = 10, r=6, $fn=6);
	        translate([2, 0, 5])cube([10, 12, 10], center = true);
	   	}
	}
	
    translate([0,-1,22]) rotate([48,0,0])translate([-0.4, 0.125, -1]){
		// bolt hole
    	cylinder(h = 30, r=1.65);
		// square nut slot
	    translate([-5, 0, 5.9])cube([15.8, 5.8, 2.2], center = true);
	}
    ...
    translate([-10,8.5-1,21]) rotate([48,0,0]) cube([20,20,20]);
